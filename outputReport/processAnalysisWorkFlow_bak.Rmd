...
#Repeat Level Analysis With Normalized Model
 By default the FDR procedure is Benjami-Hochberg, and when collapsing repeats we discard any repeats that have less than 2 CPM across samples.
```{r repeat_normalized, warning=FALSE,echo=FALSE,fig.height=9,fig.width=7}


rpt.tnx<-collapseTranscripts(annotatedKexp,read.cutoff=2)
...
ruvRWA<-repeatWiseAnalysis(annotatedKexp,
                      design=ruvDesign,
                       how="cpm",
                       p.cutoff=0.05,
                       fold.cutoff=1,
...
ruvLimmad<-as.data.frame(ruvRWA$top)
ruvLimmad<- ruvLimmad[(order(ruvLimmad$adj.P.Val)),]
#for subsetting
lg<-which(colnames(ruvLimmad)=="logFC")
aveEx<-which(colnames(ruvLimmad)=="AveExpr")
...
colnames(ruvAdvaita)<-c("ID","adj.P.Val","P.Value","t","B","logFC")


#save output
write.table(ruvAdvaita,file=paste0(appSession$gwa_outputPath,"/NormalizedrepeatWiseAnalysis.advaita.txt"),quote=FALSE,row.names=FALSE,col.names=TRUE,sep="\t")
```

```{r repeat_ruv,warning=FALSE,echo=FALSE,fig.height=9,fig.width=7}
#heatmap
rpt.tpm<-collapseTpm(annotatedKexp,"tx_id")
df.rpt<-as.data.frame(rpt.tpm,stringsAsFactors=FALSE)
    if(nrow(ruvLimmad)>50){
    rpt.targets<-rownames(ruvLimmad)[1:50]
    }
    if(nrow(ruvLimmad)<=50) {
    rpt.targets<-rownames(ruvLimmad)
    }
...
rh.rpt<-Heatmap(log(1+rpt.mt),name="log(1+tpm)",cluster_rows=rpt.dend,cluster_columns=rpt.pv$hclust,column_title="RUV Repeats(BH) Biotypes P.Val 0.05",row_names_gp=gpar(fontsize=6)) +
Heatmap(features(annotatedKexp)[rownames(rpt.mt)]$tx_biotype,name="tx_biotype",width=unit(5,"mm")) +
Heatmap(features(annotatedKexp)[rownames(rpt.mt)]$gene_biotype,name="rpt_family",width=unit(5,"mm"))
rh.rpt
}


if(nrow(rpt.mt)<29){
rh.rpt<-Heatmap(log(1+rpt.mt),name="log(1+tpm)",column_title="RUV Repeats(BH) Biotypes P.Val 0.05",row_names_gp=gpar(fontsize=6)) +
Heatmap(features(annotatedKexp)[rownames(rpt.mt)]$tx_biotype,name="tx_biotype",width=unit(5,"mm")) +
Heatmap(features(annotatedKexp)[rownames(rpt.mt)]$gene_biotype,name="rpt_family",width=unit(5,"mm"))
rh.rpt
}
...
```{r arkas_repeat_plot_unnormalized,  warning=FALSE, echo=FALSE, fig.height=9, fig.width=7}

repeats<-fitRepeats(annotatedKexp,ruvDesign)
volcanoplot(repeats$fit,coef=2,highlight=10,names=rownames(repeats$fit$t))

title(main="Top 10 Normalized D.E. Repeats")

```



#Repeat Level Analysis Without Removal of Un Wanted Variance Model
Here we plot the differential expression of detected repeats with a standard design matrix (no RUV).  By default the FDR procedure is Benjami-Hochberg, and when collapsing repeats we discard any repeats that have less than 2 CPM across samples.
```{r repeat_withoutnormalized, warning=FALSE,echo=FALSE,fig.height=9,fig.width=7}


rpt.tnx<-collapseTranscripts(annotatedKexp,read.cutoff=1)
...
RWA<-repeatWiseAnalysis(annotatedKexp,
                      design=design,
                       how="cpm",
                       p.cutoff=0.05,
                       fold.cutoff=1,
...
no.ruvLimmad<-as.data.frame(RWA$top)
no.ruvLimmad<- no.ruvLimmad[(order(no.ruvLimmad$adj.P.Val)),]
#for subsetting
lg<-which(colnames(no.ruvLimmad)=="logFC")
aveEx<-which(colnames(no.ruvLimmad)=="AveExpr")
tId<- which(colnames(no.ruvLimmad)=="t")
adjP<- which(colnames(no.ruvLimmad)=="adj.P.Val")
BId<- which(colnames(no.ruvLimmad)=="B")
pValId<-which(colnames(no.ruvLimmad)=="P.Value")


no.ruvAdvaita<-cbind(rownames(no.ruvLimmad),
                no.ruvLimmad[,adjP],
                no.ruvLimmad[,pValId],
                no.ruvLimmad[,tId],
                no.ruvLimmad[,BId],
...
colnames(no.ruvAdvaita)<-c("ID","adj.P.Val","P.Value","t","B","logFC")


#save output
write.table(no.ruvAdvaita,file=paste0(appSession$gwa_outputPath,"/unNormalizedrepeatWiseAnalysis.advaita.txt"),quote=FALSE,row.names=FALSE,col.names=TRUE,sep="\t")
```

```{r repeat_withoutruv,warning=FALSE,echo=FALSE,fig.height=9,fig.width=7}
#heatmap
un.rpt.tpm<-collapseTpm(annotatedKexp,"tx_id")
un.df.rpt<-as.data.frame(un.rpt.tpm,stringsAsFactors=FALSE)
    if(nrow(no.ruvLimmad)>50){
    un.rpt.targets<-rownames(no.ruvLimmad)[1:50]
    }
    if(nrow(no.ruvLimmad)<=50) {
    un.rpt.targets<-rownames(no.ruvLimmad)
    }

un.df.rpt<-un.df.rpt[rownames(un.df.rpt)%in%un.rpt.targets,]
un.rpt.mt<-un.df.rpt
un.rpt.mt<-as.matrix(un.rpt.mt)


if(nrow(un.rpt.mt)>=28){
un.rpt.pv<-pvclust(log(1+un.rpt.mt),nboot=100)
un.rpt.dend<-dendsort(hclust(dist(log(1+un.rpt.mt))),isReverse=TRUE)

h.rpt<-Heatmap(log(1+un.rpt.mt),name="log(1+tpm)",cluster_rows=un.rpt.dend,cluster_columns=un.rpt.pv$hclust,column_title="Repeats(BH) Biotypes P.Val 0.05",row_names_gp=gpar(fontsize=6)) +
Heatmap(features(annotatedKexp)[rownames(un.rpt.mt)]$tx_biotype,name="tx_biotype",width=unit(5,"mm")) +
Heatmap(features(annotatedKexp)[rownames(un.rpt.mt)]$gene_biotype,name="rpt_family",width=unit(5,"mm"))
h.rpt
 }


if( nrow(un.rpt.mt)<28) { 
h.rpt<-Heatmap(log(1+un.rpt.mt),name="log(1+tpm)",column_title="Repeats(BH) Biotypes P.Val 0.05",row_names_gp=gpar(fontsize=6)) +
Heatmap(features(annotatedKexp)[rownames(un.rpt.mt)]$tx_biotype,name="tx_biotype",width=unit(5,"mm")) +
Heatmap(features(annotatedKexp)[rownames(un.rpt.mt)]$gene_biotype,name="rpt_family",width=unit(5,"mm"))
h.rpt
}
...
#Enrichment Analysis
Enrichment is done using speedSage client package which accelerates Gene-Set Enrichment Analysis.Note that enrichment assumes that the variances between groups is not equal.
